...
const file = {
    ...
    md5: "99995678901234567890123456789999",
    name: "example.png",
...
describe("/file/delete/permanent POST", () => {
    let response;
        ...
        blobStream.on("finish", () => {
            (async function insert() {
                ...
                response = chakram.request(
                    "POST",
                    "http://localhost:9000/file/delete/permanent",
                    {
                        headers: {
    ...
    it("Should really deleted the file in the storage", () => {
        return chakram.waitFor([
            (async function checkDeletedPermanently() {
                let fileExists = await bucket.file(remoteFileName).exists()[0];

                expect(fileExists).to.be.undefined;
            })()
        ]);
    });
